...
char https_version_str[] = "HTTP/1.1";

char* async_net_https_method_strings[] = {
    "ACL",
    "*",
    "BASELINE-CONTROL",
    "BIND",
...
enum async_net_https_methods async_net_https_method_binary_search(char* input_string){
    ...
    while(left_index <= right_index){
        ...
        int string_compare_result = strncmp(async_net_https_method_strings[middle_index], input_string, method_str_len);
...
char* async_net_https_method_enum_find(enum async_net_https_methods curr_method){
    return async_net_https_method_strings[curr_method];
}
...
void async_net_https_message_template_init(
    ...
    strncpy(msg_template_ptr->http_version, https_version_str, sizeof(https_version_str));
    msg_template_ptr->wrapped_tls_socket = socket_ptr;
...
void async_net_https_message_template_clear(async_net_https_message_template* msg_template_ptr){
    ...
    strncpy(
        ...
        async_net_https_method_enum_find(msg_template_ptr->current_method), 
...
int https_is_chunked_checker(async_net_https_message_template* msg_template_ptr){
