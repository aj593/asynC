#include "async_net_https_request.h"
...
char https_localhost_str[] = "localhost";

typedef struct client_side_http_transaction_info client_side_http_transaction_info;

typedef struct async_net_https_incoming_response {
    async_net_https_incoming_message incoming_response;
    int status_code;
    char* status_code_str;
    char* status_message;
} async_net_https_incoming_response;

typedef struct async_net_https_outgoing_request {
    async_net_https_outgoing_message outgoing_message_info;
    int connecting_port;
    char* host;
    void (*response_handler)(async_net_https_incoming_response*, void* arg);
    void* response_arg;
    int is_socket_closed;
    event_node* request_node;
} async_net_https_outgoing_request;

typedef struct client_side_http_transaction_info {
    async_net_https_outgoing_request* request_info;
    async_net_https_incoming_response* response_info;
} client_side_http_transaction_info;

typedef struct connect_attempt_info {
    async_net_https_outgoing_request* http_request_info;
    char** ip_list;
    int total_ips;
    int curr_index;
} connect_attempt_info;

void response_data_emit_data(async_net_https_incoming_response* res, async_byte_buffer* curr_buffer);
void http_parse_response_task(void* arg);
void after_https_parse_response(void* parse_data, void* cb_arg);
void async_net_https_outgoing_request_write_head(async_net_https_outgoing_request* outgoing_request_ptr);
void https_after_request_dns_callback(char** ip_list, int num_ips, void * arg);
void https_request_connection_handler(async_tls_socket* newly_connected_socket, void* arg);
...
/*
void async_net_https_request_options_destroy(http_request_options* http_options_ptr){
    async_byte_buffer_destroy(http_options_ptr->header_buffer);
    ht_destroy(http_options_ptr->table_ptr);
}
*/

 void outgoing_https_request_init(
    async_net_https_outgoing_request* new_http_request,
    http_request_options* options_ptr,
    void (*response_handler)(async_net_https_incoming_response*, void*),
    void* arg
){
    async_net_https_message_template* template_ptr = 
    ...
    async_tls_socket* new_socket = async_tls_socket_create(NULL, 0);
    if(new_socket == NULL){
        return;
    }
...
void async_net_https_outgoing_request_destroy(async_net_https_outgoing_request* http_request_to_destroy){
    async_net_https_outgoing_message_destroy(&http_request_to_destroy->outgoing_message_info);
}

void https_incoming_response_init(async_net_https_incoming_response* new_response, async_tls_socket* socket_ptr){
    async_net_https_message_template* template_msg_ptr = &new_response->incoming_response.incoming_msg_template_info;
...
void async_net_https_incoming_response_destroy(async_net_https_incoming_response* response_to_destroy){
    async_net_https_incoming_message_destroy(&response_to_destroy->incoming_response);
}
...
int https_str_starts_with(char* whole_string, char* starting_string){
...
void async_net_https_outgoing_request_write(
    ...
    if(!writing_request->outgoing_message_info.was_header_written){
        async_net_https_client_request_set_header(writing_request, "Host", writing_request->host);
    }
...
void async_net_https_request_set_method_and_url(async_net_https_outgoing_request* http_request, enum async_net_https_methods curr_method, char* url){
    async_net_https_message_template* template_ptr = &http_request->outgoing_message_info.incoming_msg_template_info;

    template_ptr->current_method = curr_method;
    strncpy(
        template_ptr->request_method, 
        async_net_https_method_enum_find(curr_method), 
...
char* async_net_https_request_parse(async_net_https_outgoing_request* new_http_request, char* host_url, int* host_str_len_ptr){
...
async_net_https_outgoing_request* async_net_https_request(
    ...
    int single_req_res_block_size = sizeof(async_net_https_outgoing_request) + sizeof(async_net_https_incoming_response);
    void* request_and_response_block = calloc(1, single_req_res_block_size);

    async_net_https_outgoing_request* new_http_request = (async_net_https_outgoing_request*)request_and_response_block;
    
    async_net_https_message_template* msg_template_ptr = 
        &new_http_request->outgoing_message_info.incoming_msg_template_info;

    msg_template_ptr->current_method = curr_http_method;
    strncpy(msg_template_ptr->request_method, async_net_https_method_enum_find(curr_http_method), REQUEST_METHOD_STR_LEN);
    
    int host_str_len;
    char* host_str = async_net_https_request_parse(new_http_request, host_url, &host_str_len);

    outgoing_https_request_init(new_http_request, options, response_handler, arg);
    ...
    async_dns_lookup(host_str_copy, https_after_request_dns_callback, new_http_request);
...
void async_net_https_outgoing_request_write_head(async_net_https_outgoing_request* outgoing_request_ptr){
    if(!outgoing_request_ptr->outgoing_message_info.was_header_written){
        async_net_https_client_request_set_header(outgoing_request_ptr, "Host", outgoing_request_ptr->host);
    }
    
    async_net_https_outgoing_message_write_head(&outgoing_request_ptr->outgoing_message_info);
}

void https_after_request_dns_callback(char** ip_list, int num_ips, void* arg){
    connect_attempt_info* new_connect_info = (connect_attempt_info*)malloc(sizeof(connect_attempt_info));
    new_connect_info->http_request_info = (async_net_https_outgoing_request*)arg;
    new_connect_info->ip_list = ip_list;
    new_connect_info->total_ips = num_ips;
    new_connect_info->curr_index = 0;

    //TODO: write http info here instead of in https_request_connection_handler?
    /*async_tcp_socket* new_socket = */async_tls_socket_connect(
        new_connect_info->http_request_info->outgoing_message_info.incoming_msg_template_info.wrapped_tls_socket,
        new_connect_info->ip_list[new_connect_info->curr_index++],
        new_connect_info->http_request_info->connecting_port,
        https_request_connection_handler,
        new_connect_info
    );
...
void https_socket_close_handler(async_tls_socket* socket, int success, void* arg){
    ...
    async_net_https_outgoing_request_destroy(transaction_info->request_info);
    async_net_https_incoming_response_destroy(transaction_info->response_info);

    free(transaction_info->request_info);
}

void https_socket_data_handler(async_tls_socket* socket, async_byte_buffer* data_buffer, void* arg);

void https_request_connection_handler(async_tls_socket* newly_connected_socket, void* arg){
    connect_attempt_info* connect_info = (connect_attempt_info*)arg;
    async_net_https_outgoing_request* request_info = connect_info->http_request_info;
    free(connect_info->ip_list);
    free(connect_info);

    async_net_https_outgoing_request_write_head(request_info);

    //TODO: initialize socket pointers for request and response here too?
    client_side_http_transaction_info transaction_info = {
        .request_info  = request_info,
        .response_info = (async_net_https_incoming_response*)(request_info + 1)
    };
    ...
    async_net_https_incoming_message* incoming_msg_ptr = 
        &transaction_info.response_info->incoming_response;
    incoming_msg_ptr->header_data_handler = https_socket_data_handler;
    ...
    async_tls_socket_on_data(newly_connected_socket, https_socket_data_handler, new_http_transaction_node->data_ptr, 0, 0);
    async_tls_socket_on_close(newly_connected_socket, https_socket_close_handler, new_http_transaction_node->data_ptr, 0, 0);
}
...
void https_socket_data_handler(async_tls_socket* socket, async_byte_buffer* data_buffer, void* arg){
    ...
    int CRLF_check_and_parse_attempt_ret = 
        double_CRLF_check_and_enqueue_parse_task(
            ...
            socket,
            data_buffer,
            https_socket_data_handler,
            after_https_parse_response,
            client_http_info
        );
    ...
    https_incoming_response_init(client_http_info->response_info, socket);
}

void after_https_parse_response(void* parser_data, void* arg){
    client_side_http_transaction_info* transaction_info = 
        (client_side_http_transaction_info*)arg;

    //TODO: make separate emit_response function for this?
    void(*response_handler)(async_net_https_incoming_response*, void*) = transaction_info->request_info->response_handler;
    async_net_https_incoming_response* response = transaction_info->response_info;
    ...
    async_net_https_incoming_message_emit_end(&response->incoming_response);
}
...
void async_net_https_request_end(async_net_https_outgoing_request* outgoing_request){
    async_net_https_outgoing_message_end(&outgoing_request->outgoing_message_info);
}
...
void async_net_https_outgoing_request_add_trailers(async_net_https_outgoing_request* req, ...){
    ...
    async_net_https_outgoing_message_add_trailers(&req->outgoing_message_info, trailer_list);
}