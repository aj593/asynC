...
void async_net_https_outgoing_message_start_line_write(async_net_https_outgoing_message* outgoing_msg_ptr);
void async_net_https_outgoing_message_headers_write(async_net_https_outgoing_message* outgoing_msg_ptr);
...
void async_net_https_outgoing_message_destroy(async_net_https_outgoing_message* outgoing_msg){
    async_net_https_message_template_destroy(&outgoing_msg->incoming_msg_template_info);
}
...
void async_net_https_outgoing_message_set_trailer_header(async_net_https_outgoing_message* outgoing_msg_ptr){
    async_net_https_message_template* msg_template = &outgoing_msg_ptr->incoming_msg_template_info;
...
void async_net_https_outgoing_message_write_head(async_net_https_outgoing_message* outgoing_msg_ptr){
    ...
    async_net_https_outgoing_message_set_trailer_header(outgoing_msg_ptr);

    async_net_https_outgoing_message_start_line_write(outgoing_msg_ptr);
    async_net_https_outgoing_message_headers_write(outgoing_msg_ptr);
...
void async_net_https_outgoing_message_write_trailers(async_net_https_message_template* msg_template){
    ...
    for(int i = 0; i < async_util_vector_size(msg_template->trailer_vector); i++){
        ...
        async_tls_socket_write(
            msg_template->wrapped_tls_socket,
            header_buffer,
            whole_header_len - 1,
            NULL,
            NULL
...
void async_net_https_outgoing_message_write(
    ...
    async_net_https_outgoing_message_write_head(outgoing_msg_ptr);

    if(num_bytes == 0 && !is_terminating_msg){
        return;
    }

    async_net_https_message_template* template_ptr = &outgoing_msg_ptr->incoming_msg_template_info;
    ...
    if(template_ptr->is_chunked){
        if(is_terminating_msg){
            async_net_https_outgoing_message_write_trailers(template_ptr);
        }

        async_tls_socket_write(template_ptr->wrapped_tls_socket, CRLF_STR, CRLF_LEN, NULL, NULL);
    }
}
...
void async_net_https_outgoing_message_end(async_net_https_outgoing_message* outgoing_msg_ptr){
    ...
    async_net_https_outgoing_message_write(outgoing_msg_ptr, "", 0, NULL, NULL, 1);
    
    outgoing_msg_ptr->has_ended = 1;
    outgoing_msg_ptr->was_header_written = 0;
    async_net_https_message_template_clear(&outgoing_msg_ptr->incoming_msg_template_info);
}
...
void async_net_https_outgoing_message_headers_write(async_net_https_outgoing_message* outgoing_msg_ptr){
    ...
    async_tls_socket_write(msg_template->wrapped_tls_socket, CRLF_STR, CRLF_LEN, NULL, NULL);
}